...
// This function will get the users Outbox emails
...
// This function will get the users Inbox emails
...
// this function formats what we get back so its easier to map the data to the database
...
/*
        This function will get us the number of outbox messages sent based on matching ids
        and unique ids also
    */
const getTotalResponses = async () => {
  ...
  const getCountWithMatchingIds = await Promise.all(
    getOutbox.map(async outbox => {
      ...
      const getInboxConvIds = await SERA.findOne({
        ...
        .lean()
        .exec();
        // if we have matching ids then increment
      if (
        getInboxConvIds &&
  ...
  // here we filter the matching ids
  ...
  const sortData = normalizeData.filter(outbox => {
    // find the id and check if its in the array
...
router.get("/", async (req, res) => {
  ...
  if (accessToken && userName) {
    ...
    try {
      ...
      // Get the n newest messages from inbox
      ...
      // toSave holds the inbox and outbox
      const toSave = [
        ...format(inbox.value, "inbox"),
        ...format(outbox.value, "outbox")
      ];

      // emails now holds all emails
      parms.emails = toSave;
